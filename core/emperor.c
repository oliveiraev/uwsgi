...
int vassal_attr_get_multi(struct uwsgi_instance *c_ui, char *attr,
                          int (*func)(struct uwsgi_instance *, char *, void *),
                          void *data) {
  if (!attr)
    return -1;
  struct uwsgi_dyn_dict *attrs = c_ui->attrs;
...
static char **vassal_new_argv(struct uwsgi_instance *n_ui, int *slot_to_free) {
  ...
  // set args
  vassal_argv[0] =
      uwsgi.emperor_wrapper ? uwsgi.emperor_wrapper : uwsgi.binary_path;
  char *wrapper_attr = vassal_attr_get(n_ui, uwsgi.emperor_wrapper_attr);
  if (wrapper_attr)
    vassal_argv[0] = wrapper_attr;
...
static int on_demand_bind(char *socket_name) {
  ...
  if (is_tcp) {
    ...
    *is_tcp = ':';
    ...
    addr_len =
        strlen(socket_name) + ((void *)us.sa_un.sun_path - (void *)&us.sa_un);
  }
...
int uwsgi_emperor_is_valid(char *name) {
  ...
  if (uwsgi_endswith(name, ".xml") || uwsgi_endswith(name, ".ini") ||
      uwsgi_endswith(name, ".yml") || uwsgi_endswith(name, ".yaml") ||
      uwsgi_endswith(name, ".js") || uwsgi_endswith(name, ".json") ||
      has_extra_extension(name)) {
...
static char *emperor_check_on_demand_socket(char *filename,
  ...
  if (uwsgi.emperor_on_demand_extension) {
    ...
    return uwsgi_concat4n(uwsgi.emperor_on_demand_directory,
                          strlen(uwsgi.emperor_on_demand_directory), "/", 1,
                          start_of_vassal_name, last_dot - start_of_vassal_name,
                          ".socket", 7);
    ...
    char *cmd =
        uwsgi_concat4(uwsgi.emperor_on_demand_exec, " \"", filename, "\"");
  ...
  while ((de = readdir(dir)) != NULL) {
    ...
    if (uwsgi.emperor_tyrant && uwsgi.emperor_tyrant_nofollow) {
      ...
      if (lstat(de->d_name, &lst)) {
        ...
        if (ui_current) {
          uwsgi_log("!!! availability of file %s changed. stopping the "
                    "instance... !!!\n",
                    de->d_name);
    ...
    if (ui_current) {
      ...
      if (uwsgi.emperor_tyrant) {
        if (t_uid != ui_current->uid || t_gid != ui_current->gid) {
          uwsgi_log("!!! permissions of file %s changed. stopping the "
                    "instance... !!!\n",
                    de->d_name);
          emperor_stop(ui_current);
          continue;
      ...
      if (uwsgi.emperor_force_config_pipe) {
        char *config = uwsgi_simple_file_read(de->d_name);
        if (config) {
          emperor_add_with_attrs(ues, de->d_name, st.st_mtime, config,
                                 strlen(config), t_uid, t_gid, socket_name,
                                 attrs);
        } else {
          uwsgi_log_verbose("[emperor] unable to read %s\n", de->d_name);
        }
      } else {
        emperor_add_with_attrs(ues, de->d_name, st.st_mtime, NULL, 0, t_uid,
                               t_gid, socket_name, attrs);
      }
      if (socket_name)
        free(socket_name);
    }
  ...
  for (i = 0; i < (int)g.gl_pathc; i++) {
    ...
    if (ui_current) {
      ...
      if (uwsgi.emperor_force_config_pipe) {
        char *config = uwsgi_simple_file_read(g.gl_pathv[i]);
        if (config) {
          emperor_add_with_attrs(ues, g.gl_pathv[i], st.st_mtime, config,
                                 strlen(config), t_uid, t_gid, socket_name,
                                 attrs);
        } else {
          uwsgi_log_verbose("[emperor] unable to read %s\n", g.gl_pathv[i]);
        ...
        emperor_add_with_attrs(ues, g.gl_pathv[i], st.st_mtime, NULL, 0, t_uid,
                               t_gid, socket_name, attrs);
      }
      if (socket_name)
        free(socket_name);
...
void uwsgi_register_imperial_monitor(
    char *name, void (*init)(struct uwsgi_emperor_scanner *),
...
static void royal_death(int signum) {
  ...
  if (uwsgi.vassals_stop_hook) {
    ...
    while (c_ui) {
      ...
      int stop_hook_ret =
          uwsgi_run_command_and_wait(uwsgi.vassals_stop_hook, c_ui->name);
...
static void emperor_stats(int signum) {
  ...
  while (c_ui) {
    ...
    uwsgi_log(
        ...
        c_ui->name, (long long)c_ui->last_mod, c_ui->status, c_ui->loyal,
        c_ui->zerg);
...
void emperor_back_to_ondemand(struct uwsgi_instance *c_ui) {
  ...
  if (c_ui->pid != -1) {
    if (write(c_ui->pipe[0], uwsgi.emperor_graceful_shutdown ? "\2" : "\0",
              1) != 1) {
...
static void emperor_push_config(struct uwsgi_instance *c_ui) {
  ...
  if (c_ui->use_config) {
    ...
    if (write(c_ui->pipe_config[0], &uh, 4) != 4) {
      ...
      if (write(c_ui->pipe_config[0], c_ui->config, c_ui->config_len) !=
          (long)c_ui->config_len) {
...
void emperor_respawn(struct uwsgi_instance *c_ui, time_t mod) {
  ...
  if (c_ui->pid == -1 && c_ui->on_demand_fd > -1) {
    ...
    uwsgi_log_verbose(
        ...
        c_ui->name);
    if (uwsgi_hooks_run_and_return(uwsgi.hook_as_on_config_vassal,
                                   "as-on-config-vassal", c_ui->name, 0)) {
      emperor_curse(c_ui);
    }
    return;
  }
...
void emperor_add(struct uwsgi_emperor_scanner *ues, char *name, time_t born,
                 char *config, uint32_t config_size, uid_t uid, gid_t gid,
                 char *socket_name) {
  emperor_add_with_attrs(ues, name, born, config, config_size, uid, gid,
                         socket_name, NULL);
}

void emperor_add_with_attrs(struct uwsgi_emperor_scanner *ues, char *name,
                            time_t born, char *config, uint32_t config_size,
                            uid_t uid, gid_t gid, char *socket_name,
                            struct uwsgi_dyn_dict *attrs) {
  ...
  // blacklist check
  struct uwsgi_emperor_blacklist_item *uebi =
      uwsgi_emperor_blacklist_check(name);
  if (uebi) {
    uint64_t i_micros = (uebi->last_attempt.tv_sec * 1000ULL * 1000ULL) +
                        uebi->last_attempt.tv_usec + uebi->throttle_level;
    if (i_micros > micros) {
      return;
    }
  }
  ...
  if (uwsgi.emperor_command_socket) {
    if (uwsgi.emperor_wait_for_command &&
        !uwsgi_string_list_has_item(uwsgi.emperor_wait_for_command_ignore, name,
                                    strlen(name))) {
      n_ui->suspended = 1;
      uwsgi_log("[uwsgi-emperor] %s -> \"wait-for-command\" instance detected, "
                "waiting for the spawn command ...\n",
                name);
      return;
    }
  }

  // ok here we check if we need to bind to the specified socket or continue
  // with the activation
  if (socket_name) {
    ...
    uwsgi_log("[uwsgi-emperor] %s -> \"on demand\" instance detected, waiting "
              "for connections on socket \"%s\" ...\n",
              name, socket_name);
    if (uwsgi_hooks_run_and_return(uwsgi.hook_as_on_demand_vassal,
                                   "as-on-demand-vassal", name, 0)) {
      emperor_del(n_ui);
    }
    return;
  }
...
static void vassal_fork_server_parser_hook(char *key, uint16_t key_len,
...
*/
static pid_t emperor_connect_to_fork_server(char *socket,
                                            struct uwsgi_instance *n_ui) {
  ...
  while (vassal_argv[counter]) {
    if (!error && uwsgi_buffer_u16le(ub, strlen(vassal_argv[counter])))
      error = 1;
    if (!error && uwsgi_buffer_append(ub, vassal_argv[counter],
                                      strlen(vassal_argv[counter])))
      error = 1;
  ...
  if (error) {
    uwsgi_log_verbose(
        "[uwsgi-emperor] %s: unable to complete fork-server session\n",
        n_ui->name);
    goto end;
  }
  ...
  // fix uwsgi header
  if (uwsgi_buffer_set_uh(ub, 35, modifier2_mask))
    goto end;
  ...
  // the response could contain various info, currently we only need the "pid"
  // attribute
  size_t buf_len = uwsgi.page_size;
  char *buf = uwsgi_malloc(buf_len);
  uint8_t modifier1 = 0;
  uint8_t modifier2 = 0;
  int ret = uwsgi_read_with_realloc(fd, &buf, &buf_len, uwsgi.socket_timeout,
                                    &modifier1, &modifier2);
  if (ret) {
    free(buf);
    uwsgi_log_verbose(
...
int uwsgi_emperor_vassal_start(struct uwsgi_instance *n_ui) {
  ...
  if (uwsgi_hooks_run_and_return(uwsgi.hook_as_emperor_before_vassal,
                                 "as-emperor-before-vassal", NULL, 0)) {
    return -1;
  }
  ...
  char *fork_server_attr =
      vassal_attr_get(n_ui, uwsgi.emperor_fork_server_attr);
  if (fork_server_attr)
    fork_server = fork_server_attr;
  // a new uWSGI instance will start
  if (fork_server &&
      !uwsgi_string_list_has_item(uwsgi.vassal_fork_base, n_ui->name,
                                  strlen(n_ui->name))) {
    ...
    pid = clone((int (*)(void *))uwsgi_emperor_spawn_vassal,
                stack + PTHREAD_STACK_MIN, SIGCHLD | uwsgi.emperor_clone,
                (void *)n_ui);
  }
#endif
  else {
  ...
  if (pid < 0) {
    ...
    close(n_ui->pipe[1]);
    n_ui->pipe[1] = -1;
    /* THE ON-DEMAND file descriptor is left mapped to the emperor to allow
       fast-respawn
    ...
    // exec hooks have access to all of the currently defined vars +
    // UWSGI_VASSAL_PID, UWSGI_VASSAL_UID, UWSGI_VASSAL_GID, UWSGI_VASSAL_CONFIG
    uwsgi_hooks_run(uwsgi.hook_as_emperor, "as-emperor", 0);
    struct uwsgi_string_list *usl;
    uwsgi_foreach(usl, uwsgi.mount_as_emperor) {
      uwsgi_log("mounting \"%s\" (as-emperor for vassal \"%s\" pid: %d uid: %d "
                "gid: %d)...\n",
    ...
    uwsgi_foreach(usl, uwsgi.umount_as_emperor) {
      uwsgi_log("un-mounting \"%s\" (as-emperor for vassal \"%s\" pid: %d uid: "
                "%d gid: %d)...\n",
                usl->value, n_ui->name, n_ui->pid, n_ui->uid, n_ui->gid);
    ...
    uwsgi_foreach(usl, uwsgi.exec_as_emperor) {
      uwsgi_log("running \"%s\" (as-emperor for vassal \"%s\" pid: %d uid: %d "
                "gid: %d)...\n",
                usl->value, n_ui->name, n_ui->pid, n_ui->uid, n_ui->gid);
      char *argv[4];
    ...
    uwsgi_foreach(usl, uwsgi.call_as_emperor4) {
      void (*func)(char *, pid_t, uid_t, gid_t) =
          dlsym(RTLD_DEFAULT, usl->value);
      if (!func) {
        uwsgi_log("unable to call function \"%s\"\n", usl->value);
      } else {
    ...
    uwsgi_hooks_setns_run(uwsgi.hook_as_emperor_setns, n_ui->pid, n_ui->uid,
                          n_ui->gid);
#endif
...
static int uwsgi_emperor_spawn_vassal(struct uwsgi_instance *n_ui) {
...
#ifdef UWSGI_CAP
#if defined(CAP_LAST_CAP) && defined(PR_CAPBSET_READ) &&                       \
  ...
  if (uwsgi.vassals_start_hook) {
    ...
    int start_hook_ret =
        uwsgi_run_command_and_wait(uwsgi.vassals_start_hook, n_ui->name);
...
void emperor_build_scanners() {
  ...
  glob_t g;
  while (usl) {
    struct uwsgi_imperial_monitor *uim =
        imperial_monitor_get_by_scheme(usl->value);
    if (uim) {
      ...
      if (!glob(usl->value, GLOB_MARK | GLOB_NOCHECK, NULL, &g)) {
        if (g.gl_pathc == 1 &&
            g.gl_pathv[0][strlen(g.gl_pathv[0]) - 1] == '/') {
          globfree(&g);
          goto dir;
        }
        globfree(&g);
...
void emperor_loop() {
  ...
  if (uwsgi.emperor_use_fork_server || uwsgi.emperor_subreaper ||
      uwsgi.emperor_fork_server_attr) {
  ...
  if (uwsgi.emperor_use_fork_server || uwsgi.emperor_subreaper ||
      uwsgi.emperor_fork_server_attr) {
    uwsgi_log("*** DANGER: your kernel misses PR_SET_CHILD_SUBREAPER feature, "
              "required by the fork server ***\n");
    uwsgi_log("*** your Emperor will not be able to correctly wait() on "
              "vassals ***\n");
  }
#endif
  ...
  if (uwsgi.emperor_stats) {
    ...
    if (tcp_port) {
      ...
      uwsgi.emperor_stats_fd =
          bind_to_tcp(uwsgi.emperor_stats, uwsgi.listen_queue, tcp_port);
      uwsgi.no_defer_accept = current_defer_accept;
    } else {
      uwsgi.emperor_stats_fd =
          bind_to_unix(uwsgi.emperor_stats, uwsgi.listen_queue,
  ...
  if (uwsgi.emperor_trigger_socket) {
    ...
    if (tcp_port) {
      uwsgi.emperor_trigger_socket_fd = bind_to_tcp(
          uwsgi.emperor_trigger_socket, uwsgi.listen_queue, tcp_port);
    } else {
      uwsgi.emperor_trigger_socket_fd =
          bind_to_unix(uwsgi.emperor_trigger_socket, uwsgi.listen_queue,
                       uwsgi.chmod_socket, uwsgi.abstract_socket);
    }
  ...
  if (uwsgi.emperor_command_socket) {
    ...
    if (udp_port) {
      uwsgi.emperor_command_socket_fd =
          bind_to_udp(uwsgi.emperor_command_socket, 0, 0);
    } else {
      uwsgi.emperor_command_socket_fd =
          bind_to_unix_dgram(uwsgi.emperor_command_socket);
    }
  ...
  for (;;) {
    ...
    for (i = 0; i < nevents; i++) {
      ...
      if (uwsgi.emperor_stats && uwsgi.emperor_stats_fd > -1 &&
          interesting_fd == uwsgi.emperor_stats_fd) {
        emperor_send_stats(uwsgi.emperor_stats_fd);
        continue;
      }

      if (uwsgi.emperor_command_socket &&
          uwsgi.emperor_command_socket_fd > -1 &&
          interesting_fd == uwsgi.emperor_command_socket_fd) {
      ...
      if (ui_current) {
        ...
        // retry if needed
        if (rlen < 0 && uwsgi_is_again())
          continue;
        if (rlen <= 0) {
          // SAFE
          event_queue_del_fd(uwsgi.emperor_queue, interesting_fd,
                             event_queue_read());
          if (ui_current->status > 0) {
            // temporarily set frequency to a low value , so we can eventually
            // fast-restart the instance
          ...
          // heartbeat can be used for spotting blocked instances
          ...
          } else if (byte == 30 && uwsgi.emperor_broodlord > 0 &&
                     uwsgi.emperor_broodlord_count < uwsgi.emperor_broodlord) {
            uwsgi_log_verbose(
            ...
            emperor_add(ui_current->scanner, zerg_name, uwsgi_now(), NULL, 0,
                        ui_current->uid, ui_current->gid, NULL);
            free(zerg_name);
          } else if (byte == 5) {
        ...
        if (ui_current) {
          event_queue_del_fd(uwsgi.emperor_queue, ui_current->on_demand_fd,
                             event_queue_read());
...
#ifdef UWSGI_DEBUG
        uwsgi_log("%d %d %d %d\n", ui_current->last_heartbeat,
                  uwsgi.emperor_heartbeat,
                  ui_current->last_heartbeat + uwsgi.emperor_heartbeat,
                  uwsgi_now());
#endif
        if ((ui_current->last_heartbeat + uwsgi.emperor_heartbeat) <
            uwsgi_now()) {
          uwsgi_log("[emperor] vassal %s sent no heartbeat in last %d seconds, "
                    "brutally respawning it...\n",
  ...
  recheck:
    ...
    if (uwsgi.notify) {
      if (snprintf(notification_message, 64,
                   "The Emperor is governing %d vassals", has_children) >= 34) {
    ...
    if (diedpid < 0) {
      // it looks like it happens when OOM is triggered to Linux cgroup, but it
      // could be a uWSGI bug :P by the way, fallback to a clean situation...
      if (errno == ECHILD) {
    ...
    while (ui_current->ui_next) {
      ...
      if (diedpid > 0 && ui_current->pid == diedpid) {
        if (ui_current->status == 0) {
          // respawn an accidentally dead instance if its exit code is not
          // UWSGI_EXILE_CODE
          if (WIFEXITED(waitpid_status) &&
            ...
            emperor_add(ui_current->scanner, ui_current->name,
                        ui_current->last_mod, config, ui_current->config_len,
                        ui_current->uid, ui_current->gid, socket_name);
            emperor_del(ui_current);
            // temporarily set frequency to 1, so we can eventually fast-restart
            // the instance
            freq = 1;
          }
          break;
        ...
        // back to on_demand mode ...
          ...
          uwsgi_log("[uwsgi-emperor] %s -> back to \"on demand\" mode, waiting "
                    "for connections on socket \"%s\" ...\n",
        ...
        if (ui_current->pid == -1) {
          ...
          if (kill(ui_current->pid, SIGKILL) < 0) {
            uwsgi_error("[emperor] kill()");
            // delete the vassal, something is seriously wrong better to not
            // leak memory...
            emperor_del(ui_current);
          }
          break;
...
static void emperor_manage_command_parser(char *key, uint16_t keylen, char *val,
...
static void emperor_command_spawn(struct emperor_command *ec) {
  if (ec->vassal_len == 0) {
    uwsgi_log("[uwsgi-emperor] ERROR: the \"spawn\" command requires a vassal "
              "name\n");
    return;
  }
...
static void emperor_command_respawn(struct emperor_command *ec) {
  if (ec->vassal_len == 0) {
    uwsgi_log("[uwsgi-emperor] ERROR: the \"respawn\" command requires a "
              "vassal name\n");
    return;
  }
  char *name = uwsgi_concat2n(ec->vassal, ec->vassal_len, "", 0);
...
static void emperor_command_stop(struct emperor_command *ec) {
  if (ec->vassal_len == 0) {
    uwsgi_log(
        "[uwsgi-emperor] ERROR: the \"stop\" command requires a vassal name\n");
    return;
  }
  char *name = uwsgi_concat2n(ec->vassal, ec->vassal_len, "", 0);
  struct uwsgi_instance *vassal = emperor_get(name);
...
static void emperor_command_log(struct emperor_command *ec) {
  if (ec->msg_len == 0) {
    uwsgi_log(
        "[uwsgi-emperor] ERROR: the \"log\" command requires a message\n");
    return;
  }
  uwsgi_log("%.*s\n", ec->msg_len, ec->msg);
}
...
static void emperor_send_stats(int fd) {
  ...
  if (uwsgi_stats_keylong_comma(us, "emperor_tyrant",
                                (unsigned long long)uwsgi.emperor_tyrant))
    goto end0;

  if (uwsgi_stats_keylong_comma(us, "throttle_level",
                                (unsigned long long)emperor_throttle_level /
                                    1000))
    goto end0;
  ...
  while (c_ui) {
    ...
    if (uwsgi_stats_keylong_comma(us, "born", (unsigned long long)c_ui->born))
      goto end0;
    if (uwsgi_stats_keylong_comma(us, "last_mod",
                                  (unsigned long long)c_ui->last_mod))
      goto end0;
    if (uwsgi_stats_keylong_comma(us, "last_heartbeat",
                                  (unsigned long long)c_ui->last_heartbeat))
      goto end0;
    ...
    if (uwsgi_stats_keylong_comma(us, "ready", (unsigned long long)c_ui->ready))
      goto end0;
    if (uwsgi_stats_keylong_comma(us, "accepting",
                                  (unsigned long long)c_ui->accepting))
      goto end0;
    if (uwsgi_stats_keylong_comma(us, "last_loyal",
                                  (unsigned long long)c_ui->last_loyal))
      goto end0;
    if (uwsgi_stats_keylong_comma(us, "last_ready",
                                  (unsigned long long)c_ui->last_ready))
      goto end0;
    if (uwsgi_stats_keylong_comma(us, "last_accepting",
                                  (unsigned long long)c_ui->last_accepting))
      goto end0;
    if (uwsgi_stats_keylong_comma(us, "first_run",
                                  (unsigned long long)c_ui->first_run))
      goto end0;
    if (uwsgi_stats_keylong_comma(us, "last_run",
                                  (unsigned long long)c_ui->last_run))
      goto end0;
    if (uwsgi_stats_keylong_comma(us, "cursed",
                                  (unsigned long long)c_ui->cursed_at))
      goto end0;
    if (uwsgi_stats_keylong_comma(us, "zerg", (unsigned long long)c_ui->zerg))
      goto end0;

    if (uwsgi_stats_keyval_comma(us, "on_demand",
                                 c_ui->socket_name ? c_ui->socket_name : ""))
      goto end0;

    if (uwsgi_stats_keylong_comma(us, "suspended",
                                  (unsigned long long)c_ui->suspended))
      goto end0;

    if (uwsgi_stats_keylong_comma(us, "adopted",
                                  (unsigned long long)c_ui->adopted))
      goto end0;
  ...
  while (uebi) {
    ...
    if (uwsgi_stats_keylong_comma(us, "throttle_level",
                                  uebi->throttle_level / 1000))
      goto end0;

    if (uwsgi_stats_keylong_comma(us, "attempt",
                                  (unsigned long long)uebi->attempt))
      goto end0;

    if (uwsgi_stats_keylong_comma(
            us, "first_attempt",
            (unsigned long long)uebi->first_attempt.tv_sec))
      goto end0;

    if (uwsgi_stats_keylong(us, "last_attempt",
                            (unsigned long long)uebi->last_attempt.tv_sec))
      goto end0;
...
void uwsgi_emperor_simple_do_with_attrs(struct uwsgi_emperor_scanner *ues,
                                        char *name, char *config, time_t ts,
                                        uid_t uid, gid_t gid, char *socket_name,
                                        struct uwsgi_dyn_dict *attrs) {
  ...
  if (ui_current) {
    ...
    if (uwsgi.emperor_tyrant) {
      if (uid != ui_current->uid || gid != ui_current->gid) {
        uwsgi_log("[emperor-tyrant] !!! permissions of vassal %s changed. "
    ...
    if (ts > ui_current->last_mod) {
      // now we need a special check for allowing an instance to move to
      // "on_demand" mode (and back) allowing means "stopping the instance"
      if ((!ui_current->socket_name && ui_current->on_demand_fd == -1) &&
          socket_name) {
        uwsgi_log("[uwsgi-emperor] %s -> requested move to \"on demand\" mode "
                  "for socket \"%s\" ...\n",
                  name, socket_name);
        emperor_stop(ui_current);
        return;
      } else if ((ui_current->socket_name && ui_current->on_demand_fd > -1) &&
                 !socket_name) {
        uwsgi_log("[uwsgi-emperor] %s -> asked for leaving \"on demand\" mode "
                  "for socket \"%s\" ...\n",
    ...
    if (attrs)
      emperor_add_with_attrs(ues, name, ts, new_config, new_config_len, uid,
                             gid, socket_name, attrs);
    else
...
void uwsgi_emperor_simple_do(struct uwsgi_emperor_scanner *ues, char *name,
                             char *config, time_t ts, uid_t uid, gid_t gid,
                             char *socket_name) {
  uwsgi_emperor_simple_do_with_attrs(ues, name, config, ts, uid, gid,
                                     socket_name, NULL);
}
...
void uwsgi_master_manage_emperor_proxy(int server_fd, int emperor_fd,
                                       int emperor_fd_config, int socket_fd) {
