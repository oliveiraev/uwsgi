...
int uwsgi_signal_handler(struct wsgi_request *wsgi_req, uint8_t sig) {
  ...
  if (uwsgi.master_process) {
    if (use->wid != 0 && use->wid != uwsgi.mywid) {
      uwsgi_log("[uwsgi-signal] you have registered this signal in worker %d "
                "memory area, only that process will be able to run it\n",
                use->wid);
      return -1;
    }
  }
  // in lazy mode (without a master), only the same worker will be able to run
  // handlers
  else if (uwsgi.lazy) {
...
int uwsgi_register_signal(uint8_t sig, char *receiver, void *handler,
                          uint8_t modifier1) {
  ...
  if (use->receiver[0] == 0) {
    uwsgi_log("[uwsgi-signal] signum %d registered (wid: %d modifier1: %d "
              "target: default, any worker)\n",
  ...
  if (uwsgi.mywid == 0) {
    ...
    for (i = 1; i <= uwsgi.numproc; i++) {
      int pos = (i * 256);
      memcpy(&uwsgi.shared->signal_table[pos], &uwsgi.shared->signal_table[0],
             sizeof(struct uwsgi_signal_entry) * 256);
    }
  }
...
int uwsgi_add_file_monitor(uint8_t sig, char *filename) {
  ...
  if (ushared->files_monitored_cnt < 64) {
    ...
    // fill the fmon table, the master will use it to add items to the event
    // queue
    memcpy(ushared->files_monitored[ushared->files_monitored_cnt].filename,
...
int uwsgi_add_timer_hr(uint8_t sig, int secs, long nsecs) {
  ...
  if (ushared->timers_cnt < 64) {
    ...
    // fill the timer table, the master will use it to add items to the event
    // queue
...
void create_signal_pipe(int *sigpipe) {
  ...
  if (uwsgi.signal_bufsize) {
    if (setsockopt(sigpipe[0], SOL_SOCKET, SO_SNDBUF, &uwsgi.signal_bufsize,
                   sizeof(int))) {
      uwsgi_error("setsockopt()");
    }
    if (setsockopt(sigpipe[0], SOL_SOCKET, SO_RCVBUF, &uwsgi.signal_bufsize,
                   sizeof(int))) {
      uwsgi_error("setsockopt()");
    }

    if (setsockopt(sigpipe[1], SOL_SOCKET, SO_SNDBUF, &uwsgi.signal_bufsize,
                   sizeof(int))) {
      uwsgi_error("setsockopt()");
    }
    if (setsockopt(sigpipe[1], SOL_SOCKET, SO_RCVBUF, &uwsgi.signal_bufsize,
                   sizeof(int))) {
      uwsgi_error("setsockopt()");
    }
  }
...
int uwsgi_remote_signal_send(char *addr, uint8_t sig) {
  ...
  if (uwsgi_write_true_nb(fd, (char *)&uh, 4, uwsgi.socket_timeout))
    goto end;

  if (uwsgi_read_whole_true_nb(fd, (char *)&uh, 4, uwsgi.socket_timeout))
    goto end;
  close(fd);
...
int uwsgi_signal_send(int fd, uint8_t sig) {
  ...
  if (write(fd, &sig, 1) != 1) {
    if (errno == EAGAIN || errno == EWOULDBLOCK) {
      ...
      uwsgi_log("*** SIGNAL QUEUE IS FULL: buffer size %d bytes (you can tune "
                "it with --signal-bufsize) ***\n",
                so_bufsize);
    } else {
...
void uwsgi_route_signal(uint8_t sig) {
  ...
  if (use->receiver[0] == 0 || !strcmp(use->receiver, "worker") ||
      !strcmp(use->receiver, "worker0")) {
  ...
  // send to all active workers
    ...
    for (i = 1; i <= uwsgi.numproc; i++) {
      if (uwsgi.workers[i].pid > 0 && !uwsgi.workers[i].cheaped &&
          !uwsgi.workers[i].suspended) {
        if (uwsgi_signal_send(uwsgi.workers[i].signal_pipe[0], sig)) {
          uwsgi_log("could not deliver signal %d to worker %d\n", sig, i);
        }
      }
...
int uwsgi_receive_signal(struct wsgi_request *wsgi_req, int fd, char *name,
                         int id) {
...
destroy:
  ...
  uwsgi_log_verbose("uWSGI %s %d error: the master disconnected from this "
                    "worker. Shutting down the worker.\n",
                    name, id);
  end_me(0);
  // never here
  return 0;
