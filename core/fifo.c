...
/*
        ...
        the --master-fifo option create a unix named pipe (fifo) you can use to
   send management commands to the master:
...
static char *uwsgi_fifo_by_slot() {
  ...
  uwsgi_foreach(usl, uwsgi.master_fifo) {
    if (count == uwsgi.master_fifo_slot)
      return usl->value;
    count++;
  }
...
#define announce_fifo                                                          \
  uwsgi_log_verbose("active master fifo is now %s\n", uwsgi_fifo_by_slot())
...
int uwsgi_master_fifo_manage(int fd) {
  ...
  // fifo destroyed, recreate it
    ...
    event_queue_del_fd(uwsgi.master_queue, uwsgi.master_fifo_fd,
                       event_queue_read());
    close(fd);
