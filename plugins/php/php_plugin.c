...
struct uwsgi_option uwsgi_php_options[] = {
    ...
    {"php-ini", required_argument, 0, "set php.ini path", uwsgi_opt_php_ini,
     NULL, 0},
    {"php-config", required_argument, 0, "set php.ini path", uwsgi_opt_php_ini,
     NULL, 0},
    {"php-ini-append", required_argument, 0, "set php.ini path (append mode)",
     uwsgi_opt_add_string_list, &uphp.append_config, 0},
    {"php-config-append", required_argument, 0,
     "set php.ini path (append mode)", uwsgi_opt_add_string_list,
     &uphp.append_config, 0},
    {"php-set", required_argument, 0, "set a php config directive",
     uwsgi_opt_add_string_list, &uphp.set, 0},
    {"php-index", required_argument, 0, "list the php index files",
     uwsgi_opt_add_string_list, &uphp.index, 0},
    {"php-docroot", required_argument, 0, "force php DOCUMENT_ROOT",
     uwsgi_opt_set_str, &uphp.docroot, 0},
    {"php-allowed-docroot", required_argument, 0,
     "list the allowed document roots", uwsgi_opt_add_string_list,
     &uphp.allowed_docroot, 0},
    {"php-allowed-ext", required_argument, 0,
     "list the allowed php file extensions", uwsgi_opt_add_string_list,
     &uphp.allowed_ext, 0},
    {"php-allowed-script", required_argument, 0,
     "list the allowed php scripts (require absolute path)",
     uwsgi_opt_add_string_list, &uphp.allowed_scripts, 0},
    {"php-server-software", required_argument, 0, "force php SERVER_SOFTWARE",
     uwsgi_opt_set_str, &uphp.server_software, 0},
    {"php-app", required_argument, 0,
     "force the php file to run at each request", uwsgi_opt_set_str, &uphp.app,
     0},
    {"php-app-qs", required_argument, 0,
     "when in app mode force QUERY_STRING to the specified value + REQUEST_URI",
     uwsgi_opt_set_str, &uphp.app_qs, 0},
    {"php-fallback", required_argument, 0,
     "run the specified php script when the requested one does not exist",
     uwsgi_opt_set_str, &uphp.fallback, 0},
    {"php-fallback2", required_argument, 0,
     "run the specified php script relative to the document root when the "
     "requested one does not exist",
     uwsgi_opt_set_str, &uphp.fallback2, 0},
    {"php-fallback-qs", required_argument, 0,
     "php-fallback with QUERY_STRING set", uwsgi_opt_set_str, &uphp.fallback_qs,
     0},
#ifdef UWSGI_PCRE
    {"php-app-bypass", required_argument, 0,
     "if the regexp matches the uri the --php-app is bypassed",
     uwsgi_opt_add_regexp_list, &uphp.app_bypass, 0},
#endif
    {"php-var", required_argument, 0,
     "add/overwrite a CGI variable at each request", uwsgi_opt_add_string_list,
     &uphp.vars, 0},
    {"php-constant", required_argument, 0,
     "define a php constant for each request", uwsgi_opt_add_string_list,
     &uphp.constants, 0},
    {"php-dump-config", no_argument, 0,
     "dump php config (if modified via --php-set or append options)",
     uwsgi_opt_true, &uphp.dump_config, 0},
    {"php-exec-before", required_argument, 0,
     "run specified php code before the requested script",
     uwsgi_opt_add_string_list, &uphp.exec_before, 0},
    {"php-exec-begin", required_argument, 0,
     "run specified php code before the requested script",
     uwsgi_opt_add_string_list, &uphp.exec_before, 0},
    {"php-exec-after", required_argument, 0,
     "run specified php code after the requested script",
     uwsgi_opt_add_string_list, &uphp.exec_after, 0},
    {"php-exec-end", required_argument, 0,
     "run specified php code after the requested script",
     uwsgi_opt_add_string_list, &uphp.exec_after, 0},
    {"php-sapi-name", required_argument, 0,
     "hack the sapi name (required for enabling zend opcode cache)",
     uwsgi_opt_set_str, &uphp.sapi_name, 0},

    {"early-php", no_argument, 0,
     "initialize an early perl interpreter shared by all loaders",
     uwsgi_opt_early_php, NULL, UWSGI_OPT_IMMEDIATE},
    {"early-php-sapi-name", required_argument, 0,
     "hack the sapi name (required for enabling zend opcode cache)",
     uwsgi_opt_set_str, &uphp.sapi_name, UWSGI_OPT_IMMEDIATE},
    {"php-mount", required_argument, 0, "add a php mountpoint",
     uwsgi_opt_add_dyn_dict, &uphp.mountpoint, 0},
...
static int
sapi_uwsgi_send_headers(sapi_headers_struct *sapi_headers TSRMLS_DC) {
...
#endif
{
  ...
  while (read_bytes < count_bytes) {
    ssize_t rlen = 0;
    char *buf =
        uwsgi_request_body_read(wsgi_req, count_bytes - read_bytes, &rlen);
    if (buf == uwsgi.empty)
      break;
    if (buf) {
...
static void sapi_uwsgi_register_variables(zval *track_vars_array TSRMLS_DC) {
  ...
  if (uphp.server_software) {
    ...
    php_register_variable_safe("SERVER_SOFTWARE", uphp.server_software,
  ...
  if (wsgi_req->query_string_len > 0) {
    php_register_variable_safe("QUERY_STRING", wsgi_req->query_string,
                               wsgi_req->query_string_len,
                               track_vars_array TSRMLS_CC);
  }

  php_register_variable_safe("SCRIPT_NAME", wsgi_req->script_name,
                             wsgi_req->script_name_len,
                             track_vars_array TSRMLS_CC);
  php_register_variable_safe("SCRIPT_FILENAME", wsgi_req->file,
                             wsgi_req->file_len, track_vars_array TSRMLS_CC);

  php_register_variable_safe("DOCUMENT_ROOT", wsgi_req->document_root,
                             wsgi_req->document_root_len,
                             track_vars_array TSRMLS_CC);

  if (wsgi_req->path_info_len) {
    char *path_translated =
        ecalloc(1, wsgi_req->file_len + wsgi_req->path_info_len + 1);

    memcpy(path_translated, wsgi_req->file, wsgi_req->file_len);
    memcpy(path_translated + wsgi_req->file_len, wsgi_req->path_info,
           wsgi_req->path_info_len);
    php_register_variable_safe("PATH_TRANSLATED", path_translated,
                               wsgi_req->file_len + wsgi_req->path_info_len,
                               track_vars_array TSRMLS_CC);
  } else {
    php_register_variable_safe("PATH_TRANSLATED", "", 0,
                               track_vars_array TSRMLS_CC);
  }

  php_register_variable_safe("PHP_SELF", wsgi_req->script_name,
                             wsgi_req->script_name_len,
                             track_vars_array TSRMLS_CC);
...
void uwsgi_php_append_config(char *filename) {
  ...
  uwsgi_sapi_module.ini_entries =
      realloc(uwsgi_sapi_module.ini_entries, uphp.ini_size + file_size);
  memcpy(uwsgi_sapi_module.ini_entries + uphp.ini_size, file_content,
         file_size);
...
void uwsgi_php_set(char *opt) {
  ...
  uwsgi_sapi_module.ini_entries =
      realloc(uwsgi_sapi_module.ini_entries, uphp.ini_size + strlen(opt) + 2);
...
PHP_FUNCTION(uwsgi_worker_id) { RETURN_LONG(uwsgi.mywid); }
...
PHP_FUNCTION(uwsgi_cache_exists) {
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|s", &key, &keylen,
...
PHP_FUNCTION(uwsgi_cache_clear) {
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "|s", &cache,
                            &cachelen) == FAILURE) {
    RETURN_NULL();
  }
...
PHP_FUNCTION(uwsgi_cache_set) {
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ss|ls", &key, &keylen,
                            &value, &vallen, &expires, &cache,
                            &cachelen) == FAILURE) {
    RETURN_NULL();
  }
...
PHP_FUNCTION(uwsgi_cache_update) {
  ...
  if (!uwsgi_cache_magic_set(key, keylen, value, vallen, expires,
...
PHP_FUNCTION(uwsgi_signal) {
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "l", &long_signum) ==
      FAILURE) {
    RETURN_NULL();
  }
...
zend_function_entry uwsgi_php_functions[] = {
    PHP_FE(uwsgi_version, NULL) PHP_FE(uwsgi_setprocname, NULL) PHP_FE(
        uwsgi_worker_id, NULL) PHP_FE(uwsgi_masterpid,
                                      NULL) PHP_FE(uwsgi_signal, NULL)

        PHP_FE(uwsgi_rpc, NULL)

            PHP_FE(uwsgi_cache_get, NULL) PHP_FE(uwsgi_cache_set, NULL)
                PHP_FE(uwsgi_cache_update, NULL) PHP_FE(uwsgi_cache_del, NULL)
                    PHP_FE(uwsgi_cache_clear, NULL)
                        PHP_FE(uwsgi_cache_exists, NULL){NULL, NULL, NULL},
};
...
static zend_module_entry uwsgi_module_entry = {STANDARD_MODULE_HEADER,
                                               "uwsgi",
                                               uwsgi_php_functions,
...
static void activate_user_config(const char *filename, const char *doc_root,
  ...
  if ((entry = zend_hash_str_find_ptr(&uphp.user_config_cache, path,
                                      path_len)) == NULL) {
#else
  if (zend_hash_find(&uphp.user_config_cache, path, path_len + 1,
                     (void **)&entry) == FAILURE) {
#endif
...
#if (PHP_MAJOR_VERSION >= 7)
    entry = zend_hash_str_update_ptr(&uphp.user_config_cache, path, path_len,
                                     new_entry);
#else
    zend_hash_update(&uphp.user_config_cache, path, path_len + 1, new_entry,
                     sizeof(user_config_cache_entry), (void **)&entry);
#endif
  }
...
static int php_uwsgi_startup(sapi_module_struct *sapi_module) {
  ...
  if (php_module_startup(&uwsgi_sapi_module, &uwsgi_module_entry, 1) ==
      FAILURE) {
    return FAILURE;
  } else {
    return SUCCESS;
...
static int uwsgi_php_init(void) {
  ...
  zend_hash_init(&uphp.user_config_cache, 0, NULL,
...
int uwsgi_php_walk(struct wsgi_request *wsgi_req, char *full_path,
                   char *docroot, size_t docroot_len, char **path_info) {
...
static char *uwsgi_php_get_mount(char *path_info, uint16_t path_info_len,
                                 struct uwsgi_dyn_dict *mount_dict,
                                 int *discard_base) {
  int best_found = 0;
...
int uwsgi_php_request(struct wsgi_request *wsgi_req) {
  ...
  char *mount_docroot =
      uwsgi_php_get_mount(wsgi_req->path_info, wsgi_req->path_info_len,
                          uphp.mountpoint, &discard_base);
  ...
  // fallback to cwd
    ...
    if (!uwsgi_expand_path(wsgi_req->document_root, wsgi_req->document_root_len,
                           sanitized_docroot)) {
  ...
  if (uphp.app) {
    ...
    while (bypass) {
      if (uwsgi_regexp_match(bypass->pattern, bypass->pattern_extra,
                             wsgi_req->uri, wsgi_req->uri_len) >= 0) {
        goto oldstyle;
      }
      bypass = bypass->next;
    }
...
oldstyle:
  ...
  filename =
      uwsgi_concat4n(wsgi_req->document_root, wsgi_req->document_root_len, "/",
                     1, wsgi_req->path_info, wsgi_req->path_info_len, "", 0);
  ...
  if (uwsgi_php_walk(wsgi_req, filename, wsgi_req->document_root,
    ...
    if (uphp.fallback || uphp.fallback2) {
      if (uphp.fallback) {
        ...
        filename = uwsgi_concat2n(wsgi_req->document_root,
                                  strlen(wsgi_req->document_root),
      ...
      if (uphp.fallback_qs) {
        ...
        size_t new_qs_len =
            orig_path_info_len + fqs_len + 1 + wsgi_req->query_string_len;
  ...
  if (uphp.allowed_docroot) {
    ...
    while (usl) {
      if (!uwsgi_starts_with(real_filename, real_filename_len, usl->value,
                             usl->len)) {
        goto secure;
      }
      usl = usl->next;
...
secure:
  ...
  if (S_ISDIR(php_stat.st_mode)) {
    ...
    // add / to directories
    if (orig_path_info_len == 0 ||
        (orig_path_info_len > 0 &&
         orig_path_info[orig_path_info_len - 1] != '/')) {
  ...
  if (uphp.allowed_ext) {
    ...
    while (usl) {
      if (real_filename_len >= usl->len) {
        if (!uwsgi_strncmp(real_filename + (real_filename_len - usl->len),
                           usl->len, usl->value, usl->len)) {
          goto secure2;
        }
      }
      usl = usl->next;
...
secure2:
  ...
  if (uphp.allowed_scripts) {
    ...
    while (usl) {
      if (!uwsgi_strncmp(wsgi_req->file, wsgi_req->file_len, usl->value,
                         usl->len)) {
        goto secure3;
      }
      usl = usl->next;
...
secure3:
...
#ifdef UWSGI_DEBUG
  uwsgi_log(
      "php filename = %s script_name = %.*s (%d) document_root = %.*s (%d)\n",
      real_filename, wsgi_req->script_name_len, wsgi_req->script_name,
      wsgi_req->script_name_len, wsgi_req->document_root_len,
      wsgi_req->document_root, wsgi_req->document_root_len);
#endif
  ...
  SG(request_info).request_method =
      estrndup(wsgi_req->method, wsgi_req->method_len);
  SG(request_info).proto_num = 1001;

  SG(request_info).query_string =
      estrndup(wsgi_req->query_string, wsgi_req->query_string_len);
  SG(request_info).content_length = wsgi_req->post_cl;
  SG(request_info).content_type =
      estrndup(wsgi_req->content_type, wsgi_req->content_type_len);
  ...
  uwsgi_foreach(usl, uphp.exec_before) {
    if (zend_eval_string_ex(usl->value, NULL, "uWSGI php exec before",
                            1 TSRMLS_CC) == FAILURE)
      goto end;
  }
  ...
  uwsgi_foreach(usl, uphp.exec_after) {
    if (zend_eval_string_ex(usl->value, NULL, "uWSGI php exec after",
                            1 TSRMLS_CC) == FAILURE)
      goto end;
  }
