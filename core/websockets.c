...
uwsgi_websocket_message(struct wsgi_request *wsgi_req, char *msg, size_t len,
                        uint8_t opcode) {
...
static int uwsgi_websockets_ping(struct wsgi_request *wsgi_req) {
  if (uwsgi_response_write_body_do(wsgi_req, uwsgi.websockets_ping->buf,
                                   uwsgi.websockets_ping->pos)) {
    return -1;
  }
...
static int uwsgi_websockets_pong(struct wsgi_request *wsgi_req) {
  return uwsgi_response_write_body_do(wsgi_req, uwsgi.websockets_pong->buf,
                                      uwsgi.websockets_pong->pos);
}
...
static int uwsgi_websockets_check_pingpong(struct wsgi_request *wsgi_req) {
  ...
  if (wsgi_req->websocket_last_pong < wsgi_req->websocket_last_ping) {
    if (now - wsgi_req->websocket_last_ping > uwsgi.websockets_pong_tolerance) {
      uwsgi_log("[uwsgi-websocket] \"%.*s %.*s\" (%.*s) no PONG received in %d "
                "seconds !!!\n",
...
static int uwsgi_websocket_send_do(struct wsgi_request *wsgi_req, char *msg,
...
static int
uwsgi_websocket_send_from_sharedarea_do(struct wsgi_request *wsgi_req, int id,
  ...
  struct uwsgi_buffer *ub =
      uwsgi_websocket_message(wsgi_req, sa->area, len, opcode);
...
int uwsgi_websocket_send_from_sharedarea(struct wsgi_request *wsgi_req, int id,
                                         uint64_t pos, uint64_t len) {
  if (wsgi_req->websocket_closed) {
    return -1;
  }
  ssize_t ret =
      uwsgi_websocket_send_from_sharedarea_do(wsgi_req, id, pos, len, 0x81);
  if (ret < 0) {
    wsgi_req->websocket_closed = 1;
  }
  return ret;
...
int uwsgi_websocket_send_binary(struct wsgi_request *wsgi_req, char *msg,
                                size_t len) {
  if (wsgi_req->websocket_closed) {
    return -1;
  }
...
int uwsgi_websocket_send_binary_from_sharedarea(struct wsgi_request *wsgi_req,
                                                int id, uint64_t pos,
                                                uint64_t len) {
  if (wsgi_req->websocket_closed) {
    return -1;
  ...
  ssize_t ret =
      uwsgi_websocket_send_from_sharedarea_do(wsgi_req, id, pos, len, 0x82);
  if (ret < 0) {
    wsgi_req->websocket_closed = 1;
  }
  return ret;
...
static struct uwsgi_buffer *
uwsgi_websockets_parse(struct wsgi_request *wsgi_req) {
  // de-mask buffer
  uint8_t *ptr =
      (uint8_t *)(wsgi_req->websocket_buf->buf +
  ...
  /// Message is not complete, send empty dummy buffer to signal waiting for
  /// full message
...
static ssize_t uwsgi_websockets_recv_pkt(struct wsgi_request *wsgi_req,
                                         int nb) {

  int ret = -1;

  for (;;) {
    ssize_t rlen = wsgi_req->socket->proto_read_body(
        wsgi_req, wsgi_req->websocket_buf->buf + wsgi_req->websocket_buf->pos,
        wsgi_req->websocket_buf->len - wsgi_req->websocket_buf->pos);
    if (rlen > 0)
      return rlen;
    if (rlen == 0)
      return -1;
  ...
  wait:
    ...
    if (ret > 0) {
      rlen = wsgi_req->socket->proto_read_body(
          wsgi_req, wsgi_req->websocket_buf->buf + wsgi_req->websocket_buf->pos,
          wsgi_req->websocket_buf->len - wsgi_req->websocket_buf->pos);
      if (rlen > 0)
        return rlen;
...
static struct uwsgi_buffer *
uwsgi_websocket_recv_do(struct wsgi_request *wsgi_req, int nb) {
  ...
  for (;;) {
      ...
      case 1:
        if (wsgi_req->websocket_size == 126) {
          wsgi_req->websocket_size =
              uwsgi_be16(wsgi_req->websocket_buf->buf + 2);
        } else if (wsgi_req->websocket_size == 127) {
          wsgi_req->websocket_size =
              uwsgi_be64(wsgi_req->websocket_buf->buf + 2);
        } else {
          uwsgi_log("[uwsgi-websocket] \"%.*s %.*s\" (%.*s) BUG error in "
                    "websocket parser\n",
                    REQ_DATA);
          return NULL;
        }
        if (wsgi_req->websocket_size > (uwsgi.websockets_max_size * 1024)) {
          uwsgi_log("[uwsgi-websocket] \"%.*s %.*s\" (%.*s) invalid packet "
                    "size received: %llu, max allowed: %llu\n",
                    REQ_DATA, wsgi_req->websocket_size,
    ...
    // need more data
      ...
      if (uwsgi_buffer_ensure(wsgi_req->websocket_buf, uwsgi.page_size))
        return NULL;
...
ssize_t uwsgi_websockets_simple_send(struct wsgi_request *wsgi_req,
                                     struct uwsgi_buffer *ub) {
...
int uwsgi_websocket_handshake(struct wsgi_request *wsgi_req, char *key,
                              uint16_t key_len, char *origin,
                              uint16_t origin_len, char *proto,
                              uint16_t proto_len) {
#ifdef UWSGI_SSL
  if (!key_len) {
  ...
  if (uwsgi_response_prepare_headers(wsgi_req,
                                     "101 Web Socket Protocol Handshake", 33))
    return -1;
  if (uwsgi_response_add_header(wsgi_req, "Upgrade", 7, "WebSocket", 9))
    return -1;
  if (uwsgi_response_add_header(wsgi_req, "Connection", 10, "Upgrade", 7))
    return -1;
  ...
  if (wsgi_req->http_origin_len > 0 || origin_len > 0) {
    ...
    if (uwsgi_response_add_header(wsgi_req, "Sec-WebSocket-Origin", 20, origin,
                                  origin_len))
      return -1;
  } else {
    if (uwsgi_response_add_header(wsgi_req, "Sec-WebSocket-Origin", 20, "*", 1))
      return -1;
  }
  ...
  if (wsgi_req->http_sec_websocket_protocol_len > 0 || proto_len > 0) {
    ...
    if (uwsgi_response_add_header(wsgi_req, "Sec-WebSocket-Protocol", 22, proto,
                                  proto_len))
      return -1;
  }
  // generate websockets sha1 and encode it to base64
  if (!uwsgi_sha1_2n(key, key_len, "258EAFA5-E914-47DA-95CA-C5AB0DC85B11", 36,
                     sha1))
    return -1;
  size_t b64_len = 0;
  ...
  if (uwsgi_response_add_header(wsgi_req, "Sec-WebSocket-Accept", 20, b64,
                                b64_len)) {
    free(b64);
    return -1;
  }
...
#else
  uwsgi_log("you need to build uWSGI with SSL support to use the websocket "
            "handshake api function !!!\n");
  return -1;
#endif
}
