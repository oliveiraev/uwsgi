...
int uwsgi_register_rpc(char *name, struct uwsgi_plugin *plugin, uint8_t args,
                       void *func) {
  ...
  if (uwsgi.shared->rpc_count[uwsgi.mywid] < uwsgi.rpc_max) {
    int pos =
        (uwsgi.mywid * uwsgi.rpc_max) + uwsgi.shared->rpc_count[uwsgi.mywid];
    urpc = &uwsgi.rpc_table[pos];
  ...
  if (uwsgi.mywid == 0) {
    ...
    for (i = 1; i <= uwsgi.numproc; i++) {
      ...
      memcpy(&uwsgi.rpc_table[pos], uwsgi.rpc_table,
             sizeof(struct uwsgi_rpc) * uwsgi.rpc_max);
    }
  }
...
uint64_t uwsgi_rpc(char *name, uint8_t argc, char *argv[], uint16_t argvs[],
                   char **output) {
...
static void rpc_context_hook(char *key, uint16_t kl, char *value, uint16_t vl,
                             void *data) {
...
char *uwsgi_do_rpc(char *node, char *func, uint8_t argc, char *argv[],
                   uint16_t argvs[], uint64_t *len) {
  ...
  *len = 0;
  ...
  if (buffer_size > 0xffff) {
    uwsgi_log("RPC packet length overflow!!! Must be less than or equal to "
              "65535, have %llu\n",
              buffer_size);
    close(fd);
    return NULL;
  ...
  *bufptr++ = (uint8_t)((ulen >> 8) & 0xff);
  ...
  uint8_t modifier2 = 0;
  if (uwsgi_read_with_realloc(fd, &buffer, &rlen, uwsgi.socket_timeout, NULL,
                              &modifier2)) {
    goto error;
  }
  ...
  if (modifier2 == 5) {
    ...
    if (uwsgi_hooked_parse(buffer, rlen, rpc_context_hook, &content_len))
      goto error;
...
void uwsgi_rpc_init() {
  uwsgi.rpc_table = uwsgi_calloc_shared(
      (sizeof(struct uwsgi_rpc) * uwsgi.rpc_max) * (uwsgi.numproc + 1));
  uwsgi.shared->rpc_count =
      uwsgi_calloc_shared(sizeof(uint64_t) * (uwsgi.numproc + 1));
}
