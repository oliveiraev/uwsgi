...
void set_user_harakiri(struct wsgi_request *wsgi_req, int sec) {
  ...
  if (uwsgi.muleid > 0) {
    ...
    uwsgi.workers[uwsgi.mywid].cores[wsgi_req->async_id].user_harakiri =
        timeout;
  }
}
...
void uwsgi_apply_cap(cap_value_t *cap, int caps_count) {
  cap_value_t minimal_cap_values[] = {CAP_SYS_CHROOT, CAP_SETUID, CAP_SETGID,
                                      CAP_SETPCAP};
...
void uwsgi_as_root() {
  ...
  if (uwsgi.jail && !uwsgi.reloads) {
    ...
    uwsgi_foreach(usl, uwsgi.jail_ip4) { ujail.ip4s++; }
    ...
    ujail.ip4 = saddr;
#ifdef AF_INET6
    uwsgi_foreach(usl, uwsgi.jail_ip6) { ujail.ip6s++; }

    struct in6_addr *saddr6 =
        uwsgi_calloc(sizeof(struct in6_addr) * ujail.ip6s);
    i = 0;
  ...
  if (uwsgi.jail2 && !uwsgi.reloads) {
    struct uwsgi_string_list *usl = NULL;
    unsigned nparams = 0;
    uwsgi_foreach(usl, uwsgi.jail2) { nparams++; }
  ...
  if (uwsgi.chroot && !uwsgi.is_chrooted && !uwsgi.reloads) {
    ...
    if (uwsgi.logging_options.memory_report) {
      uwsgi_log("*** Warning, on linux system you have to bind-mount the /proc "
  ...
  if (uwsgi.pivot_root && !uwsgi.reloads) {
    ...
    if (!space) {
      uwsgi_log("invalid pivot_root syntax, new_root and put_old must be "
  ...
  uwsgi_foreach(usl, uwsgi.wait_for_interface) {
    ...
    uwsgi_log("waiting for interface %s (max %d seconds) ...\n", usl->value,
              uwsgi.wait_for_interface_timeout);
    int counter = 0;
    for (;;) {
  ...
  uwsgi_foreach(usl, uwsgi.wait_for_fs) {
    if (uwsgi_wait_for_fs(usl->value, 0))
      exit(1);
  }

  uwsgi_foreach(usl, uwsgi.wait_for_file) {
    if (uwsgi_wait_for_fs(usl->value, 1))
      exit(1);
  }

  uwsgi_foreach(usl, uwsgi.wait_for_dir) {
    if (uwsgi_wait_for_fs(usl->value, 2))
      exit(1);
  }

  uwsgi_foreach(usl, uwsgi.wait_for_mountpoint) {
    if (uwsgi_wait_for_mountpoint(usl->value))
      exit(1);
  }
...
#ifndef __RUMP__
  if (!getuid()) {
    uwsgi_log_initial("*** WARNING: you are running uWSGI as root !!! (use the "
                      "--uid flag) *** \n");
  }
#endif
...
static void close_and_free_request(struct wsgi_request *wsgi_req) {
  ...
  if (!wsgi_req->fd_closed) {
    // NOTE, if we close the socket before receiving eventually sent data,
    // socket layer will send a RST
...
void uwsgi_close_request(struct wsgi_request *wsgi_req) {
  ...
  if (wsgi_req->transformations) {
    if (uwsgi_apply_final_transformations(wsgi_req) == 0) {
      if (wsgi_req->transformed_chunk && wsgi_req->transformed_chunk_len > 0) {
        uwsgi_response_write_body_do(wsgi_req, wsgi_req->transformed_chunk,
                                     wsgi_req->transformed_chunk_len);
      }
    }
  ...
  if (wsgi_req->headers) {
    if (!wsgi_req->headers_sent && !wsgi_req->headers_size &&
  ...
  if (!wsgi_req->do_not_account_avg_rt) {
    ...
    uwsgi.workers[uwsgi.mywid].avg_response_time =
        (uwsgi.workers[uwsgi.mywid].avg_response_time + tmp_rt) / 2;
  }
...
#ifdef __linux__
  if (uwsgi.logging_options.memory_report || uwsgi.reload_on_uss ||
      uwsgi.reload_on_pss) {
  ...
  if (!wsgi_req->do_not_account) {
    ...
    uwsgi.workers[uwsgi.mywid].cores[wsgi_req->async_id].write_errors +=
        wsgi_req->write_errors;
    uwsgi.workers[uwsgi.mywid].cores[wsgi_req->async_id].read_errors +=
  ...
  uwsgi_foreach(usl, uwsgi.after_request_hooks) {
    void (*func)(struct wsgi_request *) =
        (void (*)(struct wsgi_request *))usl->custom_ptr;
    func(wsgi_req);
  }
  ...
  // some plugins expected async_id to be defined before setup
  wsgi_req->async_id = tmp_id;
  // yes, this is pretty useless but we cannot ensure all of the plugin have the
  // same behaviour
  uwsgi.workers[uwsgi.mywid].cores[wsgi_req->async_id].in_request = 0;

  if (uwsgi.max_requests > 0 &&
      uwsgi.workers[uwsgi.mywid].delta_requests >=
          (uwsgi.max_requests +
           ((uwsgi.mywid - 1) * uwsgi.max_requests_delta)) &&
      (end_of_request - (uwsgi.workers[uwsgi.mywid].last_spawn * 1000000) >=
       uwsgi.min_worker_lifetime * 1000000)) {
    goodbye_cruel_world(
        "max requests reached (%llu >= %llu)",
...
#ifdef __linux__
  if (uwsgi.reload_on_uss && (rlim_t)uss >= uwsgi.reload_on_uss &&
      (end_of_request - (uwsgi.workers[uwsgi.mywid].last_spawn * 1000000) >=
       uwsgi.min_worker_lifetime * 1000000)) {
  ...
  if (uwsgi.reload_on_pss && (rlim_t)pss >= uwsgi.reload_on_pss &&
      (end_of_request - (uwsgi.workers[uwsgi.mywid].last_spawn * 1000000) >=
       uwsgi.min_worker_lifetime * 1000000)) {
  ...
  if (uwsgi.linux_ksm > 0 &&
      (uwsgi.workers[uwsgi.mywid].requests % uwsgi.linux_ksm) == 0) {
    uwsgi_linux_ksm_map();
  }
#endif
#endif
...
void uwsgi_linux_ksm_map(void) {
  ...
  uwsgi.ksm_mappings_current_size =
      read(fd, uwsgi.ksm_mappings_current, uwsgi.ksm_buffer_size);
  close(fd);
  ...
  // we now have areas
  if (uwsgi.ksm_mappings_last_size == 0 ||
      uwsgi.ksm_mappings_current_size != uwsgi.ksm_mappings_last_size) {
    dirty = 1;
  } else {
    if (memcmp(uwsgi.ksm_mappings_current, uwsgi.ksm_mappings_last,
               uwsgi.ksm_mappings_current_size) != 0) {
      dirty = 1;
    }
  }
    ...
    for (i = 0; i < uwsgi.ksm_mappings_last_size; i++) {
      if (uwsgi.ksm_mappings_last[i] == '\n') {
        ...
        if (sscanf(ptr, "%llx-%llx %*s", &start, &end) == 2) {
          if (madvise((void *)(long)start, (size_t)(end - start),
                      MADV_MERGEABLE)) {
            errors++;
          }
        }
...
void wsgi_req_setup(struct wsgi_request *wsgi_req, int async_id,
  ...
  wsgi_req->uh = (struct uwsgi_header *)uwsgi.workers[uwsgi.mywid]
                     .cores[wsgi_req->async_id]
                     .buffer;
  wsgi_req->buffer =
      uwsgi.workers[uwsgi.mywid].cores[wsgi_req->async_id].buffer + 4;

  if (uwsgi.post_buffering > 0) {
    wsgi_req->post_buffering_buf =
        uwsgi.workers[uwsgi.mywid].cores[wsgi_req->async_id].post_buf;
  }
...
int wsgi_req_accept(int queue, struct wsgi_request *wsgi_req) {
  ...
  if (uwsgi.signal_socket > -1 && (interesting_fd == uwsgi.signal_socket ||
                                   interesting_fd == uwsgi.my_signal_socket)) {
  ...
  while (uwsgi_sock) {
    if (interesting_fd == uwsgi_sock->fd ||
        (uwsgi_sock->retry && uwsgi_sock->retry[wsgi_req->async_id]) ||
        (uwsgi_sock->fd_threads &&
         interesting_fd == uwsgi_sock->fd_threads[wsgi_req->async_id])) {
      wsgi_req->socket = uwsgi_sock;
...
int uwsgi_get_app_id(struct wsgi_request *wsgi_req, char *key, uint16_t key_len,
                     int modifier1) {
  ...
  if (app_name_len == 0 && wsgi_req) {
    ...
    if (app_name_len == 0) {
      ...
      if (uwsgi.vhost) {
        char *vhost_name =
            uwsgi_concat3n(wsgi_req->host, wsgi_req->host_len, "|", 1,
                           wsgi_req->script_name, wsgi_req->script_name_len);
        app_name_len = wsgi_req->host_len + 1 + wsgi_req->script_name_len;
        app_name = uwsgi_req_append(wsgi_req, "UWSGI_APPID", 11, vhost_name,
                                    app_name_len);
        free(vhost_name);
        if (!app_name) {
          uwsgi_log("unable to add UWSGI_APPID to the uwsgi buffer, consider "
                    "increasing it\n");
          return -1;
        }
#ifdef UWSGI_DEBUG
  ...
  for (i = 0; i < uwsgi_apps_cnt; i++) {
    ...
    uwsgi_log("searching for %.*s in %.*s %p\n", app_name_len, app_name,
              ...
              uwsgi_apps[i].callable);
#endif
    if (!uwsgi_apps[i].callable) {
      continue;
    }

    if (!uwsgi_strncmp(uwsgi_apps[i].mountpoint, uwsgi_apps[i].mountpoint_len,
...
#ifdef UWSGI_DEBUG
            uwsgi_log("mtime %d %d\n", st.st_mtime,
                      uwsgi_apps[i].touch_reload_mtime);
#endif
          }
        }
      }
...
#ifdef AF_INET6
#define ADDR_AF_INET_FAMILY(addrtype)                                          \
  (addrtype == AF_INET || addrtype == AF_INET6)
#else
...
char *magic_sub(char *buffer, size_t len, size_t *size, char *magic_table[]) {
  ...
  for (i = 0; i < len; i++) {
    if (buffer[i] == '%' && (i + 1) < len &&
        magic_table[(unsigned char)buffer[i + 1]]) {
      old_magic_buf = magic_buf;
      magic_buf = uwsgi_concat3n(
          old_magic_buf, magic_len, magic_table[(unsigned char)buffer[i + 1]],
          strlen(magic_table[(unsigned char)buffer[i + 1]]), buffer + i + 2,
          len - i - 2);
      free(old_magic_buf);
...
int uwsgi_num2str2(int num, char *ptr) { return snprintf(ptr, 11, "%d", num); }
...
char *uwsgi_split3(char *buf, size_t len, char sep, char **part1,
                   size_t *part1_len, char **part2, size_t *part2_len,
                   char **part3, size_t *part3_len) {
...
char *uwsgi_split4(char *buf, size_t len, char sep, char **part1,
                   size_t *part1_len, char **part2, size_t *part2_len,
                   char **part3, size_t *part3_len, char **part4,
                   size_t *part4_len) {
...
char *uwsgi_netstring(char *buf, size_t len, char **netstring,
...
struct uwsgi_dyn_dict *uwsgi_dyn_dict_new(struct uwsgi_dyn_dict **dd, char *key,
...
struct uwsgi_string_list *uwsgi_string_new_list(struct uwsgi_string_list **list,
                                                char *value) {
...
uwsgi_regexp_custom_new_list(struct uwsgi_regexp_list **list, char *value,
                             char *custom) {
...
char *uwsgi_string_get_list(struct uwsgi_string_list **list, int pos,
                            size_t *len) {
...
void uwsgi_string_del_list(struct uwsgi_string_list **list,
                           struct uwsgi_string_list *item) {
...
int uwsgi_run_command_putenv_and_wait(char *command, char *arg, char **envs,
                                      unsigned int nenvs) {
...
#endif
          uwsgi_dyn_dict_new(&uwsgi.mimetypes, current, ext_size, line,
                             type_size);
          entries++;
          ext_size = 0;
          current = NULL;
        ...
        if (uwsgi.exported_opts[i]->value[j] == symbol) {
          ...
          if (uwsgi.exported_opts[i]->value[j] == '(') {
...
#endif
            char *tmp_magic_key =
                uwsgi_concat2n(magic_key, has_symbol - 2, "", 0);
            ...
            uwsgi.exported_opts[i]->value = uwsgi_concat4n(
                ...
                (magic_key - 2) - uwsgi.exported_opts[i]->value, magic_val,
                strlen(magic_val), magic_key + (has_symbol - 1),
                strlen(magic_key + (has_symbol - 1)), "", 0);
#ifdef UWSGI_DEBUG
  ...
  *uwsgi.orig_argv[0] = 0;
  ...
  if (uwsgi.procname_prefix) {
    ...
    if (amount >= max_procname)
      return;
    strncat(uwsgi.orig_argv[0], uwsgi.procname_prefix,
            max_procname - (amount + 1));
  }
  ...
  if (uwsgi.procname_append) {
    ...
    if (amount >= max_procname)
      return;
    strncat(uwsgi.orig_argv[0], uwsgi.procname_append,
            max_procname - (amount + 1));
  }

  // if we fit into argv, only fill argv with spaces, otherwise use environ as
  // well
  ...
  if (uwsgi.procname_prefix) {
    if (!uwsgi.procname_append) {
      ...
      setproctitle("-%s%s%s", uwsgi.procname_prefix, name,
...
*/
void http_url_decode4(char *buf, uint16_t *len, char *dst,
                      int no_slash_decode) {

  enum { zero = 0, percent1, percent2, slash, dot, dotdot } status;
...
*/
char *uwsgi_get_var(struct wsgi_request *wsgi_req, char *key, uint16_t keylen,
                    uint16_t *len) {

  int i;

  for (i = wsgi_req->var_cnt - 1; i > 0; i -= 2) {
    if (!uwsgi_strncmp(key, keylen, wsgi_req->hvec[i - 1].iov_base,
...
struct uwsgi_app *uwsgi_add_app(int id, uint8_t modifier1, char *mountpoint,
                                int mountpoint_len, void *interpreter,
                                void *callable) {
  ...
  if (uwsgi.mywid == 0) {
    for (i = 1; i <= uwsgi.numproc; i++) {
      memcpy(&uwsgi.workers[i].apps[id], &uwsgi.workers[0].apps[id],
             sizeof(struct uwsgi_app));
  ...
  while (touch) {
    ...
    if (stat(touch->value, &tr_st)) {
      if (touch->custom && !touch->custom2) {
        ...
        uwsgi_log("unable to stat() %s, events will be triggered as soon as "
                  "the file is created\n",
                  touch->value);
        touch->custom2 = 1;
      }
...
#ifdef UWSGI_DEBUG
        uwsgi_log(
            "[uwsgi-check-touches] modification detected on %s: %llu -> %llu\n",
...
#elif defined(__FreeBSD__)
    if (cpuset_setaffinity(CPU_LEVEL_WHICH, CPU_WHICH_PID, -1, sizeof(cpuset),
                           &cpuset)) {
...
struct uwsgi_thread *
uwsgi_thread_new_with_data(void (*func)(struct uwsgi_thread *), void *data) {
...
int uwsgi_kvlist_parse(char *src, size_t len, char list_separator,
                       int kv_separator, ...) {
  size_t i;
  va_list ap;
...
fallback:
  ...
  snprintf(buf, 37, "%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x", r[0],
...
void uwsgi_additional_header_add(struct wsgi_request *wsgi_req, char *hh,
                                 uint16_t hh_len) {
...
void uwsgi_remove_header(struct wsgi_request *wsgi_req, char *hh,
                         uint16_t hh_len) {
  char *header = uwsgi_concat2n(hh, hh_len, "", 0);
...
char *uwsgi_get_header(struct wsgi_request *wsgi_req, char *hh, uint16_t len,
                       uint16_t *rlen) {
  char *key = uwsgi_malloc(len + 6);
  uint16_t key_len = len;
  char *ptr = key;
  *rlen = 0;
  if (uwsgi_strncmp(hh, len, "Content-Length", 14) &&
      uwsgi_strncmp(hh, len, "Content-Type", 12)) {
    memcpy(ptr, "HTTP_", 5);
    ptr += 5;
    key_len += 5;
  }
...
void uwsgi_fix_range_for_size(enum uwsgi_range *parsed, int64_t *from,
                              int64_t *to, int64_t size) {
...
char *uwsgi_getenv_with_default(const char *key) {
  /* VARIABLE:-DEFAULT_VALUE: if VARIABLE is unset or empty returns
   * DEFAULT_VALUE VARIABLE-DEFAULT_VALUE : if VARIABLE is unset returns
   * DEFAULT_VALUE
   */
  if (strstr(key, ":-") != NULL) {
    char *dup = strdup(key);
