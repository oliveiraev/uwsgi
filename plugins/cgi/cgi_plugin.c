...
#define kill_on_error                                                          \
  if (!uc.do_not_kill_on_error) {                                              \
    if (kill(cgi_pid, SIGKILL))                                                \
      uwsgi_error("kill()");                                                   \
  }
...
static void uwsgi_opt_add_cgi(char *opt, char *value, void *foobar) {
  ...
  if (!val) {
    ...
    uwsgi_dyn_dict_new(&uc.mountpoint, value, val - value, val + 1,
                       strlen(val + 1));
  }
}
...
struct uwsgi_option uwsgi_cgi_options[] = {
    ...
    {"cgi", required_argument, 0, "add a cgi mountpoint/directory/script",
     uwsgi_opt_add_cgi, NULL, 0},

    {"cgi-map-helper", required_argument, 0, "add a cgi map-helper",
     uwsgi_opt_add_cgi_maphelper, NULL, 0},
    {"cgi-helper", required_argument, 0, "add a cgi map-helper",
     uwsgi_opt_add_cgi_maphelper, NULL, 0},

    {"cgi-from-docroot", no_argument, 0, "blindly enable cgi in DOCUMENT_ROOT",
     uwsgi_opt_true, &uc.from_docroot, 0},

    {"cgi-buffer-size", required_argument, 0, "set cgi buffer size",
     uwsgi_opt_set_64bit, &uc.buffer_size, 0},
    {"cgi-timeout", required_argument, 0, "set cgi script timeout",
     uwsgi_opt_set_int, &uc.timeout, 0},

    {"cgi-index", required_argument, 0, "add a cgi index file",
     uwsgi_opt_add_string_list, &uc.index, 0},
    {"cgi-allowed-ext", required_argument, 0, "cgi allowed extension",
     uwsgi_opt_add_string_list, &uc.allowed_ext, 0},

    {"cgi-unset", required_argument, 0, "unset specified environment variables",
     uwsgi_opt_add_string_list, &uc.unset, 0},

    {"cgi-loadlib", required_argument, 0, "load a cgi shared library/optimizer",
     uwsgi_opt_add_string_list, &uc.loadlib, 0},
    {"cgi-optimize", no_argument, 0, "enable cgi realpath() optimizer",
     uwsgi_opt_true, &uc.optimize, 0},
    {"cgi-optimized", no_argument, 0, "enable cgi realpath() optimizer",
     uwsgi_opt_true, &uc.optimize, 0},

    {"cgi-path-info", no_argument, 0,
     "disable PATH_INFO management in cgi scripts", uwsgi_opt_true,
     &uc.path_info, 0},

    {"cgi-do-not-kill-on-error", no_argument, 0,
     "do not send SIGKILL to cgi script on errors", uwsgi_opt_true,
     &uc.do_not_kill_on_error, 0},
    {"cgi-async-max-attempts", no_argument, 0,
     "max waitpid() attempts in cgi async mode (default 10)", uwsgi_opt_set_int,
     &uc.async_max_attempts, 0},

    {"cgi-close-stdin-on-eof", no_argument, 0, "close STDIN on input EOF",
     uwsgi_opt_true, &uc.close_stdin_on_eof, 0},

    {"cgi-safe", required_argument, 0,
     "skip security checks if the cgi file is under the specified path",
     uwsgi_opt_add_string_list, &uc.cgi_safe, 0},

    {"cgi-dontresolve", no_argument, 0,
     "call symbolic link directly instead of the real path", uwsgi_opt_true,
     &uc.dontresolve, 0},
...
static char *uwsgi_cgi_get_helper(char *filename) {
  ...
  while (helpers) {
    if (len >= (size_t)helpers->keylen) {
      if (!uwsgi_strncmp((filename + len) - helpers->keylen, helpers->keylen,
                         helpers->key, helpers->keylen)) {
...
1 found
*/
static int uwsgi_cgi_check_status(struct wsgi_request *wsgi_req, char *buf,
                                  size_t len) {
...
#endif
          if (uwsgi_response_prepare_headers(wsgi_req, key + 8,
                                             header_size - 8))
            return -1;
          return 1;
        }
...
#endif
          if (uwsgi_response_prepare_headers(wsgi_req, "302 Found", 9))
            return -1;
          return 1;
        }
      }
...
static int uwsgi_cgi_parse(struct wsgi_request *wsgi_req, int fd, char *buf,
                           size_t blen) {
...
static char *uwsgi_cgi_get_docroot(char *path_info, uint16_t path_info_len,
                                   int *need_free, int *is_a_file,
                                   int *discard_base, char **script_name) {
  ...
  if (uc.has_mountpoints) {
    while (udd) {
      if (udd->vallen) {
        if (!uwsgi_starts_with(path_info, path_info_len, udd->key,
                               udd->keylen) &&
            udd->keylen > best_found) {
...
static int uwsgi_cgi_walk(struct wsgi_request *wsgi_req, char *full_path,
                          char *docroot, size_t docroot_len, int discard_base,
                          char **path_info) {
...
static int uwsgi_cgi_run(struct wsgi_request *, char *, size_t, char *, char *,
                         char *, char *, int, int);
...
static int uwsgi_cgi_request(struct wsgi_request *wsgi_req) {
  ...
  if (uc.from_docroot) {
    ...
    docroot = uwsgi_cgi_get_docroot(wsgi_req->path_info,
                                    wsgi_req->path_info_len, &need_free,
                                    &is_a_file, &discard_base, &script_name);
    if (docroot)
  ...
  if (!is_a_file) {
    ...
    *(full_path + docroot_len + 1) = 0;
    ...
    if (uwsgi_cgi_walk(wsgi_req, full_path, docroot, docroot_len, discard_base,
                       &path_info)) {
      if (need_free)
        free(docroot);
      return UWSGI_OK;
  ...
  while (allowed) {
    ...
    if (full_path_len >= allowed->len) {
      if (!uwsgi_strncmp(full_path + (full_path_len - allowed->len),
                         allowed->len, allowed->value, allowed->len)) {
        cgi_allowed = 1;
        break;
      }
    }
  ...
  int ret = uwsgi_cgi_run(wsgi_req, docroot, docroot_len, full_path, helper,
                          path_info, script_name, is_a_file, discard_base);
  if (need_free)
    free(docroot);
  return ret;
}

static int uwsgi_cgi_run(struct wsgi_request *wsgi_req, char *docroot,
                         size_t docroot_len, char *full_path, char *helper,
                         char *path_info, char *script_name, int is_a_file,
                         int discard_base) {
  ...
  if (cgi_pid > 0) {
    ...
    while (!completed) {
      ssize_t rlen =
          uwsgi_read_true_nb(cgi_pipe[0], buf, uc.buffer_size, uc.timeout);
      if (rlen > 0) {
  ...
  clear2:
    ...
    if (uwsgi.async > 0) {
      ...
      if (diedpid < 0) {
        ...
        // pass the pid of the cgi to async_plagued (the after request hook will
        // clear the process)
  ...
  for (i = 0; i < wsgi_req->var_cnt; i += 2) {
    ...
    if (putenv(uwsgi_concat3n(
            wsgi_req->hvec[i].iov_base, wsgi_req->hvec[i].iov_len, "=", 1,
  ...
  if (path_info) {
    ...
    if (wsgi_req->document_root_len > 0) {
      if (setenv("PATH_TRANSLATED",
                 uwsgi_concat3n(wsgi_req->document_root,
                                wsgi_req->document_root_len, path_info, pi_len,
                                "", 0),
                 1)) {
        uwsgi_error("setenv()");
      }
      ...
      if (setenv("PATH_TRANSLATED",
                 uwsgi_concat3n(docroot, docroot_len, path_info, pi_len, "", 0),
                 1)) {
        uwsgi_error("setenv()");
      }
    }
  ...
  if (is_a_file) {
    ...
    if (script_name && discard_base > 1) {
      if (setenv("SCRIPT_NAME",
                 uwsgi_concat2n(script_name, discard_base, "", 0), 1)) {
        uwsgi_error("setenv()");
      }
    }
  } else {
    ...
    if (setenv("SCRIPT_NAME",
               uwsgi_concat2n(wsgi_req->path_info, discard_base,
                              full_path + docroot_len,
                              strlen(full_path + docroot_len)),
               1)) {
      uwsgi_error("setenv()");
    }
  ...
  if (wsgi_req->query_string_len > 0) {
    if (!memchr(wsgi_req->query_string, '=', wsgi_req->query_string_len)) {
      ...
      char *qs = uwsgi_concat2n(wsgi_req->query_string,
                                wsgi_req->query_string_len, "", 0);
  ...
  if (wsgi_req->async_plagued > 0) {
    ...
    while (max_attempts) {
      ...
      if (diedpid < 0) {
        ...
        if (ret < 0) {
          kill_on_error if (waitpid(cgi_pid, &waitpid_status, 0) < 0) {
            uwsgi_error("waitpid()");
          }
        }
      } else {
...
#ifdef UWSGI_ROUTING
static int uwsgi_routing_func_cgi(struct wsgi_request *wsgi_req,
                                  struct uwsgi_route *ur) {

  char **subject = (char **)(((char *)(wsgi_req)) + ur->subject);
  uint16_t *subject_len = (uint16_t *)(((char *)(wsgi_req)) + ur->subject_len);

  struct uwsgi_buffer *ub_command = uwsgi_routing_translate(
      wsgi_req, ur, *subject, *subject_len, ur->data, ur->data_len);
  if (!ub_command)
    return UWSGI_ROUTE_BREAK;
  struct uwsgi_buffer *ub_helper = NULL;
  if (ur->data2_len) {
    ub_helper = uwsgi_routing_translate(wsgi_req, ur, *subject, *subject_len,
                                        ur->data2, ur->data2_len);
    if (!ub_helper) {
  ...
  char *docroot = uwsgi_concat2n(wsgi_req->document_root,
                                 wsgi_req->document_root_len, "", 0);
  uwsgi_cgi_run(wsgi_req, wsgi_req->document_root, wsgi_req->document_root_len,
                ub_command->buf, ub_helper ? ub_helper->buf : NULL, NULL, NULL,
                0, 0);
  free(docroot);
  uwsgi_buffer_destroy(ub_command);
  if (ub_helper)
    uwsgi_buffer_destroy(ub_helper);
  return UWSGI_ROUTE_BREAK;
}
...
static int uwsgi_router_cgi_helper(struct uwsgi_route *ur, char *args) {
  ...
  if (!space) {
    uwsgi_log(
        "invalid cgihelper syntax, must be \"cgihelper:helper command\"\n");
    return -1;
  }
  *space = 0;
  ur->data = space + 1;
